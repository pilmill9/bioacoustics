...
% [z, pill] =  dbXPathDomQuery(doc, 'ephemeris/entry/*');

% minval= min((illumination) );
% minval=min( [illumination(:)] );
...
%get phases of moon for this period
%num days required

start= addtodate(start,-30, 'day');
diff=round(  datenum(stop - start)/4   ) +4; 

[phaseday, lengthofphase] = dbGetLunarPhase(start,diff); 
...
for index=find(UseIndicator')
    ...
    illu(idx,1) = datenum(tod{index}); %pm
    
    day1= datestr(illu(idx,1));
    illu(idx,7)=-9;  
    
    ls=size(lengthofphase);
    
      for kk=1: ls(2)
            for j=1:lengthofphase(1)
         %lengthofphase(i) here
              if( year(day1) == year(phaseday(kk,j)) && month(day1)==month(phaseday(kk,j)) && day(day1)==day(phaseday(kk,j)) )
             %if( year(day1) == year(phaseday(daycount,j)) && month(day1)==month(phaseday(daycount,j)) && day(day1)==day(phaseday(daycount,j)) )
                illu(idx,7)=j;                 
              end
            end
      end
