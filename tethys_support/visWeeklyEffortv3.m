function datasets = visWeeklyEffortv2(queryEng, varargin)
...
groupmax=max(data(1).cum_hrs); %pm
...
for datIdx=1:length(data);
    for n=1:N_plots
        ...
        hold on
       %usr_max=true; %pm debug
       if usr_max
            visWeeklyPlotv3(groupmax, data(datIdx),granularity,xstep_length,xstep_unit,'First',plot_span(n,1),'Last',plot_span(n,2),'YMax',true);
        else
            visWeeklyPlotv3(groupmax, data(datIdx),granularity,xstep_length,xstep_unit,'First',plot_span(n,1),'Last',plot_span(n,2),'YMax', false);
        end
    end
    disp('next set')
